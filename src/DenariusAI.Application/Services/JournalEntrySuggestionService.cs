    ...
    IApplicationSettingsService settingsService) : IJournalEntrySuggestionService
{
    ...
    public async Task<JournalEntrySuggestionResultDto> SuggestAsync(JournalEntrySuggestionRequestDto request, CancellationToken cancellationToken = default)
    {
        ...
        var messages = new List<LlmMessageDto>
        {
            ...
            new("system", "A interface principal usa os tipos simples Expense, Income e Transfer. Para Expense cria uma linha a débito numa conta Expense com a categoria e uma linha a crédito na conta de pagamento. Para Income cria uma linha a débito na conta que recebe e uma linha a crédito numa conta Income com a categoria. Para Transfer debita a conta de destino e credita a conta de origem, normalmente sem categoria. Faz perguntas em linguagem comum e nunca peças ao utilizador que escolha débito ou crédito."),
            new("system", "Só devolvas status complete quando conseguires determinar com segurança a data, descrição, valor, tipo de movimento, conta ou contas necessárias e, em despesas ou rendimentos, a categoria. Se algum destes dados estiver ausente ou ambíguo, devolve status incomplete sem suggestion e faz uma pergunta curta e específica em português europeu para obter apenas a informação em falta. Usa as respostas e o histórico da conversa até o lançamento ficar completo."),
