...
public sealed class ApplicationSettingsViewModel
{
    ...
    /// <summary>Gets or sets the NVIDIA NIM model identifier.</summary>
    [Required, StringLength(150), Display(Name = "Modelo NVIDIA NIM")] public string NvidiaNimModel { get; set; } = DenariusAI.Application.Configuration.NvidiaNimDefaults.Model;
    /// <summary>Gets or sets the NVIDIA NIM HTTPS API root.</summary>
    [Required, Url, StringLength(300), Display(Name = "API base URL NVIDIA NIM")] public string NvidiaNimBaseUrl { get; set; } = DenariusAI.Application.Configuration.NvidiaNimDefaults.BaseUrl;
    ...
    /// <summary>Gets or sets whether verbose diagnostics are emitted for all model interactions.</summary>
    [Display(Name = "Ativar logs detalhados das interações com modelos")] public bool AiVerboseModelLogging { get; set; }
    ...
    /// <summary>Gets or sets the income, expense and flow analysis prompt.</summary>
    [Required, StringLength(10000), Display(Name = "Prompt de Rendimentos, Despesas e Fluxos")] public string IncomeExpenseFlowAnalysisPrompt { get; set; } = string.Empty;
    /// <summary>Gets or sets the budget execution analysis prompt.</summary>
    [Required, StringLength(10000), Display(Name = "Prompt de Orçamento e Execução Orçamental")] public string BudgetExecutionAnalysisPrompt { get; set; } = string.Empty;
    /// <summary>Gets or sets the savings and liquidity analysis prompt.</summary>
    [Required, StringLength(10000), Display(Name = "Prompt de Poupança e Liquidez")] public string SavingsLiquidityAnalysisPrompt { get; set; } = string.Empty;
    /// <summary>Gets or sets the investment portfolio analysis prompt.</summary>
    [Required, StringLength(10000), Display(Name = "Prompt de Investimentos e Património Financeiro")] public string InvestmentPortfolioAnalysisPrompt { get; set; } = string.Empty;
    /// <summary>Gets or sets the financial commitments analysis prompt.</summary>
    [Required, StringLength(10000), Display(Name = "Prompt de Compromissos Financeiros e Seguros")] public string FinancialCommitmentsAnalysisPrompt { get; set; } = string.Empty;
    ...
    /// <summary>Gets or sets the official IGCP Savings Certificates source page.</summary>
    [Required, Url, StringLength(500), Display(Name = "Página oficial do IGCP")] public string SavingsCertificateIgcpSourceUrl { get; set; } = "https://www.igcp.pt/pt/aforristas/produtos-de-aforro/certificados-de-aforro";
    /// <summary>Gets or sets the monthly IGCP publication URL template.</summary>
    [Required, StringLength(700), Display(Name = "Modelo de URL das publicações mensais")] public string SavingsCertificateIgcpPublicationUrlTemplate { get; set; } = "https://www.igcp.pt/pt/noticias/taxas-de-juro-dos-certificados-de-aforro-das-series-b-d-e-e-f-em-{month}-de-{year}";

    /// <summary>Maps validated form values to application settings.</summary>
    /// <returns>The settings to persist.</returns>
    public ApplicationSettingsDto ToDto() => new(MistralModel, MistralBaseUrl, AiMaxTokens, AiTemperature, AssistantSystemPrompt, AssistantContextMonths, AssistantMaxTransactions, AssistantHistoryMessages, JournalSuggestionSystemPrompt, JournalSuggestionHistoryMessages, ReconciliationExtractionPrompt, ReconciliationClassificationPrompt, DashboardWelcomePrompt, FinancialAnalysisPrompt, ConnectionTestPrompt, CorrespondenceMetadataPrompt, MarketDataProvider, MarketDataBaseUrl, InsuranceClipboardPrompt, SavingsCertificateClipboardPrompt, AiProvider, OllamaModel, OllamaBaseUrl, AiMaxInputBytes, AiContextGuidancePrompt, GroqCloudModel, GroqCloudBaseUrl, GroqCloudReasoningEffort, NvidiaNimModel, NvidiaNimBaseUrl, SavingsCertificateIgcpSourceUrl, SavingsCertificateIgcpPublicationUrlTemplate, AiVerboseModelLogging, IncomeExpenseFlowAnalysisPrompt, BudgetExecutionAnalysisPrompt, SavingsLiquidityAnalysisPrompt, InvestmentPortfolioAnalysisPrompt, FinancialCommitmentsAnalysisPrompt);
    ...
    public static ApplicationSettingsViewModel From(ApplicationSettingsDto value, bool configured) => new() { GroqCloudModel = value.GroqCloudModel, GroqCloudBaseUrl = value.GroqCloudBaseUrl, GroqCloudReasoningEffort = value.GroqCloudReasoningEffort, NvidiaNimModel = value.NvidiaNimModel, NvidiaNimBaseUrl = value.NvidiaNimBaseUrl, AiMaxInputBytes = value.AiMaxInputBytes, AiVerboseModelLogging = value.AiVerboseModelLogging, AiContextGuidancePrompt = value.AiContextGuidancePrompt, AiProvider = value.AiProvider, MistralModel = value.MistralModel, MistralBaseUrl = value.MistralBaseUrl, OllamaModel = value.OllamaModel, OllamaBaseUrl = value.OllamaBaseUrl, AiMaxTokens = value.AiMaxTokens, AiTemperature = value.AiTemperature, AssistantSystemPrompt = value.AssistantSystemPrompt, AssistantContextMonths = value.AssistantContextMonths, AssistantMaxTransactions = value.AssistantMaxTransactions, AssistantHistoryMessages = value.AssistantHistoryMessages, JournalSuggestionSystemPrompt = value.JournalSuggestionSystemPrompt, JournalSuggestionHistoryMessages = value.JournalSuggestionHistoryMessages, ReconciliationExtractionPrompt = value.ReconciliationExtractionPrompt, ReconciliationClassificationPrompt = value.ReconciliationClassificationPrompt, DashboardWelcomePrompt = value.DashboardWelcomePrompt, FinancialAnalysisPrompt = value.FinancialAnalysisPrompt, IncomeExpenseFlowAnalysisPrompt = value.IncomeExpenseFlowAnalysisPrompt, BudgetExecutionAnalysisPrompt = value.BudgetExecutionAnalysisPrompt, SavingsLiquidityAnalysisPrompt = value.SavingsLiquidityAnalysisPrompt, InvestmentPortfolioAnalysisPrompt = value.InvestmentPortfolioAnalysisPrompt, FinancialCommitmentsAnalysisPrompt = value.FinancialCommitmentsAnalysisPrompt, ConnectionTestPrompt = value.ConnectionTestPrompt, CorrespondenceMetadataPrompt = value.CorrespondenceMetadataPrompt, MarketDataProvider = value.MarketDataProvider, MarketDataBaseUrl = value.MarketDataBaseUrl, InsuranceClipboardPrompt = value.InsuranceClipboardPrompt, SavingsCertificateClipboardPrompt = value.SavingsCertificateClipboardPrompt, SavingsCertificateIgcpSourceUrl = value.SavingsCertificateIgcpSourceUrl, SavingsCertificateIgcpPublicationUrlTemplate = value.SavingsCertificateIgcpPublicationUrlTemplate, AiIsConfigured = configured };
}
