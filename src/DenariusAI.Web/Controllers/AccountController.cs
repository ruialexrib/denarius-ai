...
/// <param name="demoMode">The deployment-level demonstration mode configuration.</param>
/// <param name="httpClientFactory">The HTTP client factory used for external profile images.</param>
    ...
    ILogger<AccountController> logger) : Controller
{
    ...
    /// <summary>Displays the login page or redirects authenticated users to the home page.</summary>
    ...
    /// <summary>Processes the login form submission and authenticates the user.</summary>
    ...
    /// <param name="returnUrl">The local URL requested before authentication.</param>
    /// <param name="remoteError">An error returned by the external provider.</param>
    /// <returns>A redirect after successful authentication or the login view with an error.</returns>
    [AllowAnonymous, HttpGet]
    ...
    /// <summary>Returns the login view with an external authentication error.</summary>
    /// <param name="returnUrl">The local URL requested before authentication.</param>
    /// <param name="message">The error shown to the user.</param>
    ...
    private async Task<IActionResult> CompleteLoginAsync(ApplicationUser user, string? returnUrl)
    {
        ...
        TempData["SuccessMessage"] = previousLogin is null ? "Bem-vindo. Este é o seu primeiro acesso registado." : $"Bem-vindo. O seu acesso anterior foi em {FormatLogin(previousLogin.LoggedInAt)}, a partir do IP {previousLogin.IpAddress}.";
    ...
    /// <summary>Creates the login model with authentication and demonstration deployment context.</summary>
    /// <param name="returnUrl">The local URL requested before authentication.</param>
    /// <returns>The populated login view model.</returns>
    private LoginViewModel LoginModel(string? returnUrl)
    {
    ...
    /// <summary>Applies provider and public demonstration context to a login model.</summary>
    /// <param name="model">The login model to populate.</param>
    private void ApplyLoginContext(LoginViewModel model)
    {
        model.GoogleEnabled = IsGoogleConfigured;
        model.DemoModeEnabled = demoMode.Value.Enabled;
        model.DemoEmail = demoMode.Value.HasCredentials ? demoMode.Value.Email : null;
        model.DemoPassword = demoMode.Value.HasCredentials ? demoMode.Value.Password : null;
    }
    ...
    /// <summary>Synchronizes the local profile image with a trusted Google profile image.</summary>
    /// <param name="user">The authenticated application user.</param>
    /// <param name="imageUrl">The external profile image URL.</param>
    private async Task SynchronizeGoogleProfileImageAsync(ApplicationUser user, string? imageUrl)
    {
        ...
        try
        {
            using var response = await httpClientFactory.CreateClient("GoogleProfileImages").GetAsync(uri, HttpCompletionOption.ResponseHeadersRead, HttpContext.RequestAborted);
            ...
            int read;
            while ((read = await stream.ReadAsync(chunk, HttpContext.RequestAborted)) > 0) { if (buffer.Length + read > MaximumProfileImageBytes) return; await buffer.WriteAsync(chunk.AsMemory(0, read), HttpContext.RequestAborted); }
            ...
            user.ProfileImageBase64 = base64; user.ProfileImageContentType = contentType;
            var result = await userManager.UpdateAsync(user);
            if (!result.Succeeded) logger.LogWarning("The Google profile image could not be saved for user {UserId}.", user.Id);
        }
        catch (Exception exception) when (exception is HttpRequestException or IOException or TaskCanceledException) { logger.LogWarning(exception, "The Google profile image could not be downloaded for user {UserId}.", user.Id); }
    }

    /// <summary>Validates that a profile image URL belongs to Google's trusted image host.</summary>
    /// <param name="value">The candidate URL.</param>
    /// <param name="uri">The validated URI when successful.</param>
    /// <returns><see langword="true"/> when the URL is trusted.</returns>
    private static bool IsTrustedGoogleImageUrl(string? value, out Uri uri)
    {
        if (Uri.TryCreate(value, UriKind.Absolute, out var candidate) && candidate.Scheme == Uri.UriSchemeHttps && (candidate.Host.Equals("googleusercontent.com", StringComparison.OrdinalIgnoreCase) || candidate.Host.EndsWith(".googleusercontent.com", StringComparison.OrdinalIgnoreCase))) { uri = candidate; return true; }
        uri = null!; return false;
    }

    /// <summary>Logs out the current user and redirects to the login page.</summary>
    ...
    /// <summary>Displays the access denied page for unauthorized access attempts.</summary>
    /// <returns>The access denied view.</returns>
    [AllowAnonymous] public IActionResult AccessDenied() => View();

    /// <summary>Records the user's cookie consent acceptance.</summary>
    ...
    /// <summary>Records that the authenticated user acknowledged the cloud AI privacy notice for the current login session.</summary>
    /// <param name="returnUrl">The local URL to return to after acknowledgement.</param>
    /// <returns>A redirect to the requested local URL or the home page.</returns>
    [HttpPost, ValidateAntiForgeryToken]
    public IActionResult AcknowledgeCloudAiPrivacyNotice(string? returnUrl = null) { HttpContext.Session.Remove(CloudAiPrivacyNoticePolicy.SessionKey); return Url.IsLocalUrl(returnUrl) ? LocalRedirect(returnUrl) : RedirectToAction("Index", "Home"); }

    /// <summary>Displays the user's profile page with login history.</summary>
    /// <returns>A view with the user's profile.</returns>
    [HttpGet]
    public async Task<IActionResult> Profile() { var user = await userManager.GetUserAsync(User); if (user is null) return Challenge(); return View(await ProfileModelAsync(user)); }

    /// <summary>Processes profile updates submitted by the user.</summary>
    /// <param name="model">The profile view model containing updated user information.</param>
    /// <returns>A redirect to the profile page on success, or the profile view with errors.</returns>
    [HttpPost, ValidateAntiForgeryToken]
    ...
    /// <summary>Processes a password change request for the current user.</summary>
    ...
    /// <param name="enabled">Whether the widget should be visible.</param>
    ...
    public async Task<IActionResult> SetAssetBalancesWidget(bool enabled, string? returnUrl = null)
    {
        ...
        else TempData["SuccessMessage"] = enabled ? "Resumo patrimonial reativado. Ficará visível em toda a aplicação." : "Resumo patrimonial ocultado. Pode reativá-lo a qualquer momento nas Preferências.";
        return Url.IsLocalUrl(returnUrl) ? LocalRedirect(returnUrl) : RedirectToAction(nameof(Profile));
    }

    /// <summary>Maps password error codes to user-friendly error messages.</summary>
    ...
    /// <summary>Creates a profile view model for the specified user.</summary>
    /// <param name="user">The application user.</param>
    /// <returns>A view model with profile data and login history.</returns>
    private async Task<ProfileViewModel> ProfileModelAsync(ApplicationUser user) => new() { DisplayName = user.DisplayName, Email = user.Email ?? string.Empty, ShowAssetBalancesWidget = user.ShowAssetBalancesWidget, LoginHistory = await dbContext.LoginHistory.AsNoTracking().Where(item => item.UserId == user.Id).OrderByDescending(item => item.LoggedInAt).Take(10).Select(item => new LoginHistoryItemViewModel(item.LoggedInAt, item.IpAddress)).ToListAsync() };

    /// <summary>Retrieves the client's IP address from the HTTP context.</summary>
    /// <returns>The client's IP address as a string.</returns>
    private string ClientIp() { var address = HttpContext.Connection.RemoteIpAddress; if (address is null) return "Desconhecido"; if (IPAddress.IsLoopback(address)) return "127.0.0.1"; return address.IsIPv4MappedToIPv6 ? address.MapToIPv4().ToString() : address.ToString(); }

    /// <summary>Formats a login timestamp to the Lisbon timezone with a localized format.</summary>
    /// <param name="value">The UTC timestamp to format.</param>
    /// <returns>A formatted date-time string.</returns>
    private static string FormatLogin(DateTimeOffset value) { var zone = TimeZoneInfo.FindSystemTimeZoneById("Europe/Lisbon"); return TimeZoneInfo.ConvertTime(value, zone).ToString("dd/MM/yyyy 'às' HH:mm"); }
}
