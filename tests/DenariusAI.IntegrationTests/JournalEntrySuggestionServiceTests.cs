...
public sealed class JournalEntrySuggestionServiceTests
{
    ...
    public async Task CompleteSuggestionUsesOnlyCatalogIdsAndBalancedLines()
    {
        ...
        var service = new JournalEntrySuggestionService(data, data, data, data, data, data);
    ...
    private sealed class SuggestionData : ILLMService, IAccountService, ICategoryService, IFinancialGroupService, IBudgetService, IApplicationSettingsService
    {
        ...
        Task<ApplicationSettingsDto> IApplicationSettingsService.GetAsync(CancellationToken cancellationToken) => Task.FromResult(new ApplicationSettingsDto(Model, "https://api.mistral.ai/v1/", 1024, .2, "Assistant", 12, 200, 10, DenariusAI.Application.Configuration.ApplicationSettingsDefaults.JournalSuggestionPrompt, 10));
